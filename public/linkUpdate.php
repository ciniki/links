...
function ciniki_links_linkUpdate(&$ciniki) {
    ...
    $rc = ciniki_links_checkAccess($ciniki, $args['tnid'], 'ciniki.links.linkUpdate'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    $modules = $rc['modules'];
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.links.link', $args['link_id'], $args, 0x07);
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
